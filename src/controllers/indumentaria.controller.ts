...
export class IndumentariaController {
  ...
  static async getStockBajo(_req: AuthRequest, res: Response) {
    const bajos = await AppDataSource.getRepository(Indumentaria)
  ...
  static async update(req: AuthRequest, res: Response) {
    const { stockMinimo, categoria, talle, color, genero, ubicacion, proveedorId, observaciones } =
      req.body;
    const repo = AppDataSource.getRepository(Indumentaria);
    ...
    // Solo se toca modificadoPor si el request viene autenticado (no se pisa
    // el valor previo cuando no hay usuario).
    if (req.user?.id) {
      prenda.modificadoPor = await getUsuarioActual(req);
    }
  ...
  static async delete(req: AuthRequest, res: Response) {
    ...
    prenda.activo = false;
    if (req.user?.id) {
      prenda.eliminadoPor = await getUsuarioActual(req);
    }
    await repo.save(prenda);
