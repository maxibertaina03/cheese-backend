...
export class MotivoController {
  ...
  static async getAll(_req: Request, res: Response) {
    const motivos = await AppDataSource.getRepository(Motivo).find({
      where: { activo: true },
      order: { nombre: 'ASC' },
    });
    res.json(motivos);
  ...
  static async getOne(req: Request, res: Response) {
    const motivo = await AppDataSource.getRepository(Motivo).findOne({
  ...
  static async update(req: AuthRequest, res: Response) {
    ...
    const motivo = await motivoRepo.findOne({ where: { id: Number(req.params.id) } });
