// src/routes/usuario.routes.ts (nuevo archivo)
...
router.put('/:id', auth, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    ...
    const usuarioRepo = AppDataSource.getRepository(Usuario);
    const usuario = await usuarioRepo.findOneBy({ id: parseInt(id) });
    
    if (!usuario) {
      return res.status(404).json({ error: 'Usuario no encontrado' });
    }
    
    // No permitir cambiar el rol de uno mismo (opcional, por seguridad)
    if (usuario.id === (req as any).user.id) {
